from DataInterface import getDummyDataset2,getDummyDataset1,getConnect4Dataset, getCarDataset, getBalloonDataset
...
def getAverageClassificaionRate(dataset,runs=20,testSize=200,setFunc = setEntropy, infoFunc = infoGain):
    ...
    for r in xrange(runs):
        ...
        print 'Score for run %d is %.2f'%(r+1,score)
        scores.append(round(score,5))
    average = sum(scores)/float(runs)
    print 'Average classification rate over all runs: %.2f'%(average)
...
def testBalloon(setFunc = setEntropy, infoFunc = infoGain):
    """Correct classification averate rate is about 0.95"""
    examples,attrValues,labelName,labelValues = getBalloonDataset()
    print 'Testing Balloon dataset. Number of examples %d.'%len(examples)
    tree = makeTree(examples, attrValues, labelName, setFunc, infoFunc)
    f = open('balloon.out','w')
    f.write(str(tree))
    f.close()
    print 'Tree size: %d.\n'%tree.count()
    print 'Entire tree written out to balloon.out in local directory\n'
    dataset = getBalloonDataset()
    evaluation = getAverageClassificaionRate((examples,attrValues,labelName,labelValues))
    print 'Results for training set:\n%s\n'%str(evaluation)
    printDemarcation()
    return (tree,evaluation)
